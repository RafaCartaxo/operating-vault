# 01 - Análise (<ID>)

> [!info]- Navegação QA/DEV
> **README do card:** [[00 README|Abrir README do card]]  
> **Execução:** [[04 Trabalho DEV/Execuções/<ID>/00 README|README da execução]]  
> **Demanda QA:** [[<DEMANDA>|<ID> — <título>]]  
> **Plano:** [[02 - Plano de execução|02 - Plano de execução]]  
> **Implementação:** [[03 - Implementação|03 - Implementação]]  
> **Code review:** [[04 - Code review|04 - Code review]]  
> **Validação QA:** [[<VALIDACAO>|Validação QA]]

---

## Veredito

Em uma frase: a causa foi confirmada (bug) ou a melhoria é viável (melhoria), com a condição relevante.

---

## O que foi confirmado

- Evidência no código, comportamento atual e impacto.

---

## Abordagem e riscos

- Dependências, contratos, migrações, compatibilidade, segurança ou serviços externos.

---

## Alternativas descartadas

- Alternativa e motivo de não seguir.

---

## Perguntas abertas

- Nenhuma. Se houver decisão que altere escopo, regra ou aceite, voltar o card para análise.
