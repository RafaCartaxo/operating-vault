...
**Próximo passo:** nenhum — MEL-0003 concluída e validada.
...
> const defeitos = dv.pages('"03 Trabalho/QA/Nxgest/Demandas/Bugs"').where(p => String(p.pai ?? "").includes(id));
...
> dv.paragraph(`**${id || "MEL-0003"} — Retrabalho por defeitos filhos:** ${adicional} pontos (${defeitos.length} defeitos)`);
> ```

Pacote QA para `MEL-0003`:
